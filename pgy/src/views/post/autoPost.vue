<template>
  <div id="auto-post">
    ...
    <Modal v-model="RulesModal.modal" title="设置规则">
      <Form
        ref="RulesModal"
        :model="RulesModal.data"
        :rules="ValidateRules"
      ...
      >
        ...
        <FormItem label="内容：" prop="content">
          <Input
            v-model="RulesModal.data.content"
            type="textarea"
  ...
  data() {
    return {
      ...
      ValidateRules: {
        ...
        behavior: [{ required: true, message: "行为不能为空！" }],
        status: [{ required: true, message: "状态不能为空！" }],
        trigger_date: [{ required: true, message: "触发时间不能为空！" }],
    ...
    AddOver() {
      this.$refs["RulesModal"].validate(valid => {
        if (valid) {
          ...
          const url = isEdit
            ? "/backend/Autopush/autoPushUp"
            : "/backend/Autopush/autoPushAdd";
          if (isEdit) {
