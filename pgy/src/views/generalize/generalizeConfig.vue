  ...
  <div id="generalize-config">
    ...
    <div class="data-area" style="margin-bottom: 20px;">
      <Card :padding="0">
        <div class="card-tit" slot="title">
          <h3>
            <Icon type="images"></Icon>图片列表
          </h3>
          <div class="btn-box">
            <Button type="info" size="large" icon="plus-circled" @click="AddPic">添加图片</Button>
          </div>
        </div>
        <Table border :columns="PicCol" :data="PicData" :loading="picloading"></Table>
      </Card>
    </div>
    <div class="data-area">
      <Card :padding="0">
        <div class="card-tit" slot="title">
          <h3>
            <Icon type="clipboard"></Icon>渠道列表
          </h3>
          <div class="btn-box">
            <Button type="info" size="large" icon="plus-circled" @click="AddChannel">添加渠道</Button>
          </div>
        </div>
    ...
    <Modal v-model="PicModal.modal" title="设置图片" :width="600">
      ...
      >
        ...
        <FormItem label="图片：">
          <Upload
            :before-upload="HandleAvator"
            action="http://api.xrjinrong.com/backend/Auth/adminUserUppwd"
          >
    ...
    <Modal v-model="ModeModal.modal" title="设置推广渠道">
      <Form
        ref="ModeModal"
      ...
      >
        ...
        <FormItem label="外部账号名：" prop="account">
          <Input v-model="ModeModal.data.account"></Input>
        </FormItem>
         <FormItem v-show='ModeModal.url_show' label="推广短链：" >
          <Input v-model="ModeModal.data.shortUrl"></Input>
        </FormItem>
        <FormItem label="背景图：">
  ...
  data() {
    return {
      ...
      PicCol: [
        ...
        //           index: params.index,
        //           // datas: this.UserData,
        //           url: "/backend/Promoteconfig/promoteDisableImg"
        //         }
        //       })
        //     ]);
        //   }
          ...
          render: (h, params) => {
            return h(
              "Button",
              {
                ...
                on: {
                  click: () => {
                    this.CheckBigPic(params.row.shortImg);
                  }
                }
              },
              "查看大图"
            ...
            return h("div", [
              h(radioList, {
                props: {
                  ...
                  index: params.index,
                  datas: this.UserData,
                  url: "/backend/promote-config/promote-disable"
                }
              })
            ]);
          }
      ...
      ValidateRules: {
        title: [{ required: true, message: "渠道名称不能为空！" }],
  ...
  methods: {
    ...
    AddOver() {
      this.$refs["ModeModal"].validate(valid => {
        if (valid) {
          ...
          const url = isEdit
            ? "/backend/Promoteconfig/promoteEdit"
            : "/backend/Promoteconfig/promoteAdd";

          ninfo.id = this.ModeModal.id;
          // ninfo.code_color = "#ffd225";
          // ninfo.submit_color = "#ffd225";
          // ninfo.submit_text = "#ffd225";
    ...
    Delopt(row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.UploadData("/backend/Promoteconfig/promoteDel", {
            id: row.id
          }).then(() => {
            this.InitData(this.apiUrl);
          });
    ...
    PicOver() {
      this.$refs["PicModal"].validate(valid => {
        if (valid) {
          ...
          const url = isEdit
            ? "/backend/Promoteconfig/imgEdit"
            : "/backend/Promoteconfig/imgAdd";
          if (isEdit) {
    ...
    ChangeStatus(row) {
      this.$post("/backend/promote-config/promote-disable", {
        id: row.id
      }).then(response => {
        this.InitData(this.apiUrl);
      });
