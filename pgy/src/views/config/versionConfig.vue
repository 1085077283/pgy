<template>
  <div id="version-config">
    ...
    <Modal v-model="ModeModal.modal" title="添加版本信息">
      <Form
        ref="ModeModal"
...
export default {
  ...
  data() {
    return {
      ...
      ValidateRules: {
        version_sn: [{ required: true, message: "版本号不能为空！" }],
        version_address: [{ required: true, message: "版本地址不能为空！" }],
        version_desc: [{ required: true, message: "版本描述不能为空！" }]
      },
      UserData: [], //表格数据
      RowUserData: [], //获取的原始数据
      BtnData: []
  ...
  methods: {
    ...
    AddOver() {
      this.$refs["ModeModal"].validate(valid => {
        if (valid) {
          ...
          const url = isEdit
            ? "/backend/Sysconfig/sysVersionUp"
            : "/backend/Sysconfig/sysVersionAdd";
          if (isEdit) {
    ...
    Delopt(row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.UploadData("/backend/Sysconfig/sysVersionDel", {
            id: row.id
          }).then(() => {
            this.InitData(this.apiUrl);
          });
