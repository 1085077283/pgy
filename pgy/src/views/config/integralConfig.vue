    ...
    <div class="data-area">
      <Card :padding="0">
        <div class="card-tit" slot="title">
          ...
          <div class="btn-box">
            <Button type="warning" size="large" icon="wrench" @click="AddModeModal">添加配置</Button>
          </div>
        </div>
        <Table border :columns="UserCol" :data="UserData" :loading="loading"></Table>
      </Card>
    </div>
    <Modal v-model="ModeModal.modal" title="添加积分">
      <Form
        ref="ModeModal"
  ...
  data() {
    return {
      ...
      ValidateRules: {
        ...
        between_start: [{ required: true, message: "下限不能为空！" }],
        between_end: [{ required: true, message: "上限不能为空！" }]
      },
      UserData: [], //表格数据
      RowUserData: [], //获取的原始数据
      BtnData: []
