<template>
  <div id="commission-formula">
    ...
    <Card>
      ...
      <Row type="flex" justify="space-between">
        <Col :span="11">
          ...
          <Table border :columns="ParamsTable.FixedCol" :data="ParamsTable.FixedData"></Table>
        </Col>
        <Col :span="11">
          <h4 class="table-tit">配置数据（可修改参数值）</h4>
          <Table border :columns="ParamsTable.DynamicCol" :data="ParamsTable.DynamicData"></Table>
        </Col>
      </Row>
    </Card>
    <div class="data-area">
      <Card :padding="0">
        <div class="card-tit" slot="title">
          <h3>
            <Icon type="clipboard"></Icon>支付公式列表
          </h3>
          <div class="btn-box">
    ...
    <Modal v-model="ParamsModal.modal" title="设置参数">
      <Form
        ref="ParamsModal"
        :model="ParamsModal.data"
        :rules="ValidateRules"
      ...
      <div slot="footer">
        ...
        <Button
          type="primary"
          @click="AddOver('ParamsModal','/backend/Commission/resultparmEdit','/backend/Commission/resultparmAdd')"
          size="large"
        >保存</Button>
      </div>
    </Modal>
    <Modal v-model="ModeModal.modal" title="设置支付公式" :width="650">
      <Form
        ref="ModeModal"
          ...
          <div class="chose-box">
            <Select v-model="ParamsEdit" style="width:220px">
              <Option
                ...
                :value="item.name_en"
                :key="item.name_en"
              >{{ `${item.name_cn} ${item.name_en}` }}</Option>
            </Select>
            ...
            <Button type="success" @click="SymbolAdd" style="margin-left: 5px">添加</Button>
      ...
      <div slot="footer">
        ...
        <Button
          type="primary"
          @click="AddOver('ModeModal','/backend/Commission/resultsFormulaEdit','/backend/Commission/resultsFormulaAdd')"
          size="large"
        >保存</Button>
      </div>
    </Modal>
...
export default {
  ...
  data() {
    return {
      title: "业绩公式配置",
      apiUrl: "/backend/Commission/resultsFormulaList",
      auth_id: "",
      loading: true,
      ParamsModal: {
        modal: false,
      ...
      ValidateRules: {
        title_cn: [{ required: true, message: "公式标题不能为空！" }],
        title_en: [{ required: true, message: "公式英文标题不能为空！" }],
        content: [{ required: true, message: "公式内容不能为空！" }],
        name_cn: [{ required: true, message: "参数名称不能为空！" }],
        name_en: [{ required: true, message: "参数代号不能为空！" }]
      },
      UserData: [], //表格数据
      RowUserData: [], //获取的原始数据
      BtnData: []
  ...
  computed: {
    ...
    FormulaEditText() {
      ...
      if (this.FormulaEditArr.length > 0) {
        return this.FormulaEditArr.join("");
      }
    }
  },
  methods: {
    ...
    //获取基础参数
    GetBaseParams() {
      this.$fetch("/backend/Commission/resultparmList").then(d => {
    ...
    DelParams(row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.UploadData("/backend/Sysconfig/sysFormulaBDel", {
            id: row.id
          }).then(() => {
            this.GetBaseParams();
          });
    ...
    SymbolAdd() {
      this.FormulaEditArr.push(this.ParamsSymbol);
    },
    FormulaDel() {
    ...
    Delopt(row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.UploadData("/backend/Commission/resultsFormulaDel", {
            id: row.id
          });
          this.UserData.filter((item, index) => {
            if (item.id == row.id) {
