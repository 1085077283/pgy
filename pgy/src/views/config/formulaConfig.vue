  ...
  <div id="formula-config">
    ...
    <Modal v-model="ModeModal.modal" title="设置支付公式" :width="650">
      ...
      <div slot="footer">
        ...
        <Button
          type="primary"
          @click="AddOver('ModeModal','/backend/Sysconfig/sysFormulaUp','/backend/Sysconfig/sysFormulaAdd')"
          size="large"
        >保存</Button>
      </div>
    </Modal>
    <Modal v-model="ParamsModal.modal" title="设置参数">
      ...
      <div slot="footer">
        ...
        <Button
          type="primary"
          @click="AddOver('ParamsModal','/backend/Sysconfig/sysFormulaBUp','/backend/Sysconfig/sysFormulaBAdd')"
          size="large"
        >保存</Button>
      </div>
    </Modal>
...
export default {
  ...
  data() {
    return {
      ...
      ValidateRules: {
        title: [{ required: true, message: "公式标题不能为空！" }],
        title_en: [{ required: true, message: "公式英文标题不能为空！" }],
        content: [{ required: true, message: "公式内容不能为空！" }],
        namecn: [{ required: true, message: "参数名称不能为空！" }],
        nameen: [{ required: true, message: "参数代号不能为空！" }]
      },
      UserData: [], //表格数据
      RowUserData: [], //获取的原始数据
      BtnData: []
  ...
  methods: {
    ...
    Delopt(row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.UploadData("/backend/Sysconfig/sysFormulaDel", {
            id: row.id
          }).then(() => {
            this.InitData(this.apiUrl);
          });
