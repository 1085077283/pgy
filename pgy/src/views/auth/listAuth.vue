<template>
  <div id="listRole">
    ...
    <div class="data-area">
      <Table border :columns="UserCol" size="large" :data="DepData" :loading="loading"></Table>
    </div>
    <Modal v-model="AddModal" title="添加菜单">
      <Form
        ref="AddInfo"
        :model="AddInfo"
        :rules="ValidateRules"
    ...
    <Modal v-model="EditModal" title="编辑菜单">
      <Form
        ref="EditInfo"
        :model="EditInfo"
        :rules="ValidateRules"
  ...
  data() {
    return {
      ...
      ValidateRules: {
        ...
        sort: [{ required: true, message: "请填写排序数字！" }]
      },
      EditModal: false,
      EditInfo: {
        title: "",
    ...
    EditOpt(row) {
      ...
      for (let key in this.EditInfo) {
        if (key === "title") {
          ...
          this.EditInfo.hideBtn =
            row.levelNum === 3 || row.levelNum === 4 ? false : true;
        } else if (key === "mark") {
