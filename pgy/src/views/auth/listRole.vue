<template>
  <div id="listRole">
    ...
    <div class="data-area">
      <Table border :columns="RoleCol" size="large" :data="RoleData" :loading="loading"></Table>
    </div>
    <Modal v-model="AddModal" title="添加角色">
      <Form
        ref="AddInfo"
        :model="AddInfo"
        :rules="AddRules"
    ...
    <Modal v-model="EditModal" title="编辑权限">
      <Form
        ref="EditInfo"
        :model="EditInfo"
        :rules="AddRules"
    ...
    <Modal v-model="AssignModal" title="权限分配" :width="800" @on-ok="AssignOver">
          ...
          <CheckboxGroup v-model="AssignInfo.auth_id">
            <div v-for="(item,index) in AuthList" :key="item.id">
              ...
              <div class="auth-seclist" v-for="(secItem,secIndex) in item.auth" :key="secItem.id">
                ...
                <div
                  class="inline-block"
                  v-for="(thirdItem, thirdIndex) in secItem.auth"
                  :key="thirdItem.id"
                >
                  <p>
...
export default {
  ...
  data() {
    return {
      ...
      AddRules: {
        name: [{ required: true, message: "角色名称不能未空" }],
        sort: [{ required: true, message: "请填写排序数字，数字越小越靠前" }]
      },
      EditModal: false,
      EditInfo: {
        id: "",
  ...
  methods: {
    ...
    UploadData(url, info) {
      if (info.status) {
        info.status = info.status === "已启用" ? 1 : 0;
      }
