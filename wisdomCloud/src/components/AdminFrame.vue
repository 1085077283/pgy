<template>
  <div id="admin" class="layout">
    <Row type="flex">
      ...
      <Col span="21" class="layout-menu-right" :style="{minHeight: RightHeight}">
        <div class="layout-header">
          ...
          <div class="login-modify" @click="OpenCenter">
             更换密码
          </div>
          <div class="pull-left_text" >
            <div>姓名:&nbsp;  {{ username }}</div>
    ...
    <Modal v-model="CenterModal.modal" title="修改个人信息">
      <Form
        ref="CenterModal"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      ValidateRules: {
        pwd1: [{ required: true, message: "密码不能为空！" }],
        pwd2: [{ required: true, message: "请重复输入相同的密码！" }]
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    SubmitOver() {
      this.$post("/backend/Auth/adminUserUppwd", this.CenterModal.data)
        .then(d => {
          if (d.status === 1) {
